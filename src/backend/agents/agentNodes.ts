...
*Auto-generated by Vulnerability Remediation Agent*`,
      ...
      // Pass repository context from vulnerability
    ...
    return {
      prUrl,
      status: "creating_pr",
      stepLog: [logEntry("createPR", `PR created: ${prUrl} in ${vuln.repoOwner}/${vuln.repoName}`)],
    };
  } catch (err: any) {
    return {
      status: "failed",
  ...
  try {
    ...
    return {
      ...
      stepLog: [
        logEntry(
          "createTagWorkflow",
          `Tag "${result.tagName}" created in ${vuln.repoOwner}/${vuln.repoName}. Workflow triggered — awaiting user approval at ${result.workflowRunUrl}`
        ),
      ],
    };
  } catch (err: any) {
